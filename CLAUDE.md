...
```bash
python3 -m pytest tests/ -q          # 147 cases, no Qt, no network, under a second
...
`FLM_MODEL` sets only what is preloaded. FastFlowLM hot-swaps per request, so a client asking for a different model gets that one loaded instead. The tooltip resolves the resident model from `[FLM] Loading model:` paths through `resolve_model_tag()`, a fuzzy match because `Qwen3.6-35B-A3B-NPU2` is served as `qwen3.6-moe:35b-a3b`.
